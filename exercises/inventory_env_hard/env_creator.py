...
from inventory_env.wrappers import MyNormalizeObservation, GymNormalizeObservation
from inventory_env_hard.wrappers import MakeHard, MyScaleReward


def inventory_env_hard_creator(config, seed=None):
    obs_filter = config.pop("obs_filter", None)
    reward_filter = config.pop("reward_filter", None)
    env = MakeHard(InventoryEnv())
